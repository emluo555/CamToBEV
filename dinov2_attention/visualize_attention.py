...
if __name__ == '__main__':
    ...
    img = Image.open('/scratch/gpfs/el5267/CamToBEV/datasets/nuscenes-trainval/samples/CAM_FRONT/n015-2018-09-25-13-17-43+0800__CAM_FRONT__1537852954612476.jpg')
    img = img.convert('RGB')
    ...
    combined_attn = np.mean(attentions, axis=0)

    #combined_attn = (combined_attn - combined_attn.min()) / (combined_attn.max() - combined_attn.min() + 1e-8)

    combined_fname = os.path.join(output_dir, "attn-combined.png")
    plt.imsave(fname=combined_fname, arr=combined_attn, cmap='viridis', format='png')
    print(f"{combined_fname} saved.")